...
CREATE TABLE `ANNOUNCEMENT` (
  ...
  `ANM_STATUS_ID` tinyint(1) NOT NULL COMMENT '公告狀態',
  ...
  PRIMARY KEY (`ANM_ID`),
  KEY `FK_ANM_ADMINISTRATOR_ID` (`ADMINISTRATOR_ID`),
...
insert  into `ANNOUNCEMENT`(`ANM_ID`,`ADMINISTRATOR_ID`,`ANM_ORDER_ID`,`ANM_STATUS_ID`,`ANM_TITLE`,`ANM_CONTENT`,`ANM_TYPE_ID`,`ANM_START_DATE`,`ANM_END_DATE`) values 
(8000,1,2,1,'「無三不成禮」小黃卡住宿禮遇','打疫苗享折扣！自即日起至 2022/8/30前推出「無三不成禮」小黃卡專屬住宿禮遇。於入住時出示紙本疫苗卡或健保 App ，打一劑享延遲退房至中午12點、打兩劑享延遲退房至下午1點、打滿三劑則可延遲退房至下午2點，再贈送1,000元餐廳抵用券，不限平假日皆可使用，每房最多一次。',1,'2021-05-01','2022-08-31'),
(8001,1,1,1,'防疫政策說明','為因應防疫規範，建議賓客可下載「臺灣社交距離App」，並配合佩戴口罩、酒精消毒及測量體溫。此外，提醒入住賓客請務必遵守以下防疫規定：1.離開客房時請全程配戴口罩。2.避免不必要移動、活動或集會。3.房內人數以原同住家人，並依房型所登記之人數為原則。另依觀光局指示，房內禁止群聚開趴或唱歌等行為，致生傳染風險及觸法。4.自我健康監測(有症狀應就醫)。不便之處，敬祈見諒。共同守護健康，攜手努力為防疫盡一份心力！',3,'2022-03-01',NULL),
(8002,1,1,1,'防範詐騙提醒','防止新型態網路詐騙，呼籲賓客提高警覺防詐騙！！好旅提醒您，當您使用線上系統完成交易後，本飯店不會以電話通知您:因網路分期付款發生設定錯誤等理由而要求您操作ATM解除、變更付款方式或補繳金額；亦不會以電話通知要求您提供信用卡帳號或個人及銀行相關資料。請您務必提高警覺以免上當受騙，如接獲不明及可疑的電話或簡訊提出上述要求時，請勿提供任何資料，歡迎您聯繫好旅查證或並請儘速反應至警政署反詐騙諮詢專線165或內政部警政署全民防騙網遠離詐騙三『不』！不碰ATM！不說個人資料！不要回撥陌生電話！■ 不碰ATM詐騙集團會要求客戶去ATM轉換英文操作介面，是為了誘使您轉帳。ATM沒有退款、解除分期付款…等功能，請您務必小心。好旅不會任意變更已付款的訂單，不會要求您依照指示操作使用ATM。■ 不說信用卡資料 好旅不會以電話請您提供銀行帳戶、信用卡資料；亦不會以「問卷」或「中獎」形式通知您提供個人資料或要求匯款。■ 不回撥陌生電話詐騙集團會篡改電信資料，轉來電顯示或是發送簡訊，若您接獲此類可疑電話或簡訊，請與好旅查證或撥打165反詐騙諮詢專線查證，以確保安全。',3,'2021-01-01',NULL),
(8003,1,3,2,'壽星專屬 厚禮獨享','Celebrate Your Birthday with Us! 與您共慶生日美好時光! 本月壽星於現場出示證明文件認證，即贈蛋糕單片一份!',2,'2022-09-01','2022-10-30'),
(8004,1,3,0,'高壓電年度檢查休館通知','感謝您一直以來對於好旅的支持與愛護。依照政府法規，飯店將進行年度高壓電力檢查，並將於2021/11/14(一)中午 12 點至 2021/11/15(二)下午 3 點暫時關閉飯店，2021/11/15(二)下午 3 點恢復入住手續服務。造成不便，敬請見諒。',3,'2021-06-01','2021-11-16');

/*Table structure for table `ANNOUNCEMENT_PICTURE` */

DROP TABLE IF EXISTS `ANNOUNCEMENT_PICTURE`;

CREATE TABLE `ANNOUNCEMENT_PICTURE` (
  `ANM_PICTURE_ID` int NOT NULL AUTO_INCREMENT COMMENT '公告圖片ID',
  `ANM_ID` int DEFAULT NULL COMMENT '公告編號(外來鍵)',
  `ANM_PICTURE` longblob COMMENT '公告圖片',
  PRIMARY KEY (`ANM_PICTURE_ID`),
  KEY `FK_ANNOUNCEMENT_PICTURE_ANM_TYPE_ID` (`ANM_ID`),
...
/*Data for the table `ANNOUNCEMENT_PICTURE` */

insert  into `ANNOUNCEMENT_PICTURE`(`ANM_PICTURE_ID`,`ANM_ID`,`ANM_PICTURE`) values 
(9002,8000,NULL),
(9003,8001,NULL);
...
CREATE TABLE `CS_MAIL` (
  ...
  `FAQ_TYPE_NAME` varchar(4) DEFAULT NULL COMMENT '問題種類名稱',
...
CREATE TABLE `JOURNEY` (
  ...
  `JOURNEY_PICTURE` blob COMMENT '行程照片',
...
(12000,1000,11000,2,0,'2022-08-25'),
(12001,1001,11001,4,2,'2022-08-07'),
(12002,1002,11001,2,0,'2022-08-07'),
(12003,1003,11001,5,0,'2022-08-20'),
(12004,1004,11000,2,3,'2022-08-20');
...
CREATE TABLE `ROOM_USED_RECORD` (
  ...
  `SOURCE` tinyint(1) NOT NULL COMMENT '來源',
...
(4000,601,1000,'2022-01-01','2022-01-02','安妮亞',1),
(4001,301,NULL,'2022-02-01','2022-02-02',NULL,0),
(4002,501,1002,'2022-03-01','2022-03-02','紐特',1),
(4003,502,1003,'2022-04-01','2022-04-05','哈利',1),
(4004,302,1004,'2022-05-01','2022-05-03','洛伊德',1);
...
(1000,1,'2022-05-22','2022-06-22','2022-06-23','2022-05-23',2,'獨身男子的行程: 一房一行程，有修改日期的訂單，狀態已付款',5000,700),
(1001,3,'2022-04-02','2022-05-02','2022-05-09',NULL,1,'小屁孩安妮亞:一房一行程，無修改日期的訂單，狀態未付款',8000,1500),
(1002,4,'2022-04-02','2022-05-02','2022-05-09',NULL,2,'家庭的行程: 兩房有行程，無修改日期的訂單，狀態已付款',56000,1500),
(1003,2,'2022-06-01','2022-07-01','2022-07-02','2022-06-02',4,'自動取消的行程: 沒付錢，系統要自動更改修改日期的訂單，狀態已取消',5000,300),
(1004,2,'2022-06-02','2022-07-01','2022-07-02',NULL,1,'隔天趕快再訂的行程: 跟上一筆訂單內容相同，狀態已付款',5000,300),
(1005,5,'2022-04-02','2022-05-02','2022-05-09',NULL,3,'隨便建立一張已完成的訂單',154000,NULL);
...
CREATE TABLE `USER` (
  ...
  `PHONE_VERIFY_STATUS` tinyint(1) NOT NULL COMMENT '手機驗證狀態',
...
insert  into `USER`(`USER_ID`,`USER_ACCOUNT`,`USER_PASSWORD`,`USER_NAME`,`USER_BIRTHDAY`,`USER_EMAIL`,`USER_PHONE`,`PHONE_VERIFY_STATUS`,`USER_IDENTITYNUMBER`,`USER_RIGISTRATION_DATE`) values 
(1,'S43246546','A78GRENW56','紐特','1897-02-24','Newt35532@gmail.com','0911432648',1,'C123456789','2022-06-22'),
(2,'w4awg46570','b78Gwfht36','哈利','1980-07-31','Harry643408@gmail.com','0965528704',0,'A987654321','2022-06-22'),
(3,'JRIEWOJ492','fwei592j3t4','安妮亞','2018-12-31','Aniya849382@gmail.com','0988934012',1,'B047264910','2022-04-02'),
(4,'JT4I47425','fjewni583h','洛伊德','1982-01-01','Loid9506372@gmail.com','0903758512',0,'B795320765','2022-04-02'),
